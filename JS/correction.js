...
listForms.forEach((formulaire) => {
  ...
  let listInputs = formulaire.querySelectorAll(
    "input:not([type='submit']):not([type='reset']):not([type='button']),select,textarea"
  );
  let submit = formulaire.querySelector(
...
function verifEmail(listInputsValidity, formulaire) {
  ...
  if (mailSaisi != null && listemail.includes(mailSaisi.value)) {
    //modif
...
function verifPassword(listInputsValidity, formulaire) {
  ...
  if (
    listePassword.length == 2 &&
    listePassword[0].value != listePassword[1].value
  ) {
    //modif
